...
# section_tag 는 컴파일 시에만 변동 (질의 처리 중에는 사실상 고정).
# LLM unified plan 호출 직전마다 재계산하던 SQL을 캐시한다.
_KNOWN_TAGS_CACHE: tuple[float, list[str]] = (0.0, [])
_KNOWN_TAGS_TTL_SEC = 60.0
...
의도 라벨 정의:
  ...
  finance        : 실적·재무·영업이익·가이던스·리스크·ETF 보수/순자산
  relations      : 경쟁사·협력사·관련주·자회사
  theme          : 테마·섹터·업종·정책 카테고리·ETF 기초지수/추종지수
  generic        : 위에 명확히 속하지 않음

규칙:
- 임의로 코드를 만들어내지 말고, 제공된 ticker_master에 있는 코드만 사용.
- ticker가 불명확하면 tickers=[] 로 반환 (추측 금지).
- related_tags는 섹터명·키워드(HBM, SMR, 배당, 바이오시밀러, ETF, S&P500 등) 만, 완전문 문장 금지.
"""
...
def _known_tags(limit: int = 60) -> list[str]:
    """DB에 존재하는 태그 상위 N개 — LLM 에게 어휘 힌트로 제공.

    호출 빈도가 높고(매 LLM 통합 plan), 결과는 컴파일 사이에만 변경되므로
    `_KNOWN_TAGS_TTL_SEC` 동안 캐시한다.
    """
    global _KNOWN_TAGS_CACHE
    cached_at, cached = _KNOWN_TAGS_CACHE
    now = time.time()
    if cached and (now - cached_at) < _KNOWN_TAGS_TTL_SEC:
        return cached[:limit]
    try:
        with tx() as conn:
            rows = conn.execute(
        ...
        return []
    _KNOWN_TAGS_CACHE = (now, tags)
    return tags[:limit]


def reset_known_tags_cache() -> None:
    """compile 직후처럼 강제로 캐시 무효화하고 싶을 때 호출."""
    global _KNOWN_TAGS_CACHE
    _KNOWN_TAGS_CACHE = (0.0, [])
